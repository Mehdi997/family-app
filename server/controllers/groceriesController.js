 ...
 * Listes de courses, articles, budgets
 */
...
const getLists = async (req, res) => {
  try {
    ...
    // Ajouter les stats de chaque liste
    for (const list of lists) {
      const [items] = await pool.query(
        `SELECT
          COUNT(*) as total_items,
          SUM(CASE WHEN is_checked THEN 1 ELSE 0 END) as checked_items,
          COALESCE(SUM(estimated_price * quantity), 0) as estimated_total,
          COALESCE(SUM(CASE WHEN is_checked THEN actual_price * quantity ELSE 0 END), 0) as actual_total
...
const getList = async (req, res) => {
  try {
    const [lists] = await pool.query(
      'SELECT * FROM grocery_lists WHERE id = ? AND family_id = ?',
    ...
    console.error('Erreur détail liste:', error);
    res.status(500).json({ message: 'Erreur lors du chargement.' });
  }
};
...
const createList = async (req, res) => {
  try {
    ...
    const [result] = await pool.query(
      'INSERT INTO grocery_lists (family_id, name, type, budget, store, date, notes) VALUES (?, ?, ?, ?, ?, ?, ?)',
      [req.user.family_id, name, type || 'weekly', budget || null, store || null, date || null, notes || null]
    );

    res.status(201).json({ message: 'Liste créée.', id: result.insertId });
  } catch (error) {
...
const addItem = async (req, res) => {
  try {
    ...
    const [result] = await pool.query(
      'INSERT INTO grocery_items (list_id, name, quantity, unit, estimated_price, category) VALUES (?, ?, ?, ?, ?, ?)',
      [req.params.id, name, quantity || 1, unit || null, estimatedPrice || null, category || null]
    );
...
/** PUT /api/groceries/items/:itemId */
const updateItem = async (req, res) => {
  try {
    const { name, quantity, unit, estimatedPrice, actualPrice, category, isChecked } = req.body;

    await pool.query(
      `UPDATE grocery_items SET name = ?, quantity = ?, unit = ?, estimated_price = ?,
       actual_price = ?, category = ?, is_checked = ? WHERE id = ?`,
      [name, quantity, unit, estimatedPrice, actualPrice || null, category, isChecked || false, req.params.itemId]
    );
...
/** PUT /api/groceries/items/:itemId/toggle */
const toggleItem = async (req, res) => {
  try {
    const { actualPrice } = req.body;

    await pool.query(
      'UPDATE grocery_items SET is_checked = NOT is_checked, actual_price = ? WHERE id = ?',
      [actualPrice || null, req.params.itemId]
    );
...
const deleteList = async (req, res) => {
  try {
    const [result] = await pool.query(
      'DELETE FROM grocery_lists WHERE id = ? AND family_id = ?',
      [req.params.id, req.user.family_id]
    );
...
/** DELETE /api/groceries/items/:itemId */
